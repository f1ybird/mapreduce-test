...
/**
 * describe  : 计算各个城市员工的总工资
 *
 * 输入文件：file/dept.txt file/emp.txt
 * 输出文件：file/sumCitySalary.txt
 *
 * creat_user: kevin
 * creat_time: 2018/9/4 21:39
                ...
                for (Path path : paths) {
                    ...
                    if (path.toString().contains("dept")) {
                        ...
                        while (null != ((deptIdName = br.readLine()))) {
                            // 对部门文件字段进行拆分并缓存到deptMap中,其中Map中key为部门编号， value为城市名称
                            deptMap.put(deptIdName.split(Constant.COMMA_SPLIT)[0], deptIdName.split(Constant.COMMA_SPLIT)[2]);
                        }
                    }
                }
            } catch (Exception e) {
...
//        job.setJar(PathConst.JAR_OUTPUT_PATH);
        job.setMapperClass(Q4SumCitySalary.MapClass.class);
        job.setReducerClass(Q4SumCitySalary.ReduceClass.class);
        // 设置输入格式类
    ...
    public static void main(String[] args) throws Exception{
        int result = ToolRunner.run(new Configuration(), new Q4SumCitySalary(), args);
        System.exit(result);
    }
}
