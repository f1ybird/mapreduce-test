...
/**
 * describe  : 求每个部门最早进入公司的员工姓名
 *
 * 输入文件：file/dept.txt file/emp.txt
 * 输出文件：file/out_deptNumAndSalary.txt
 *
 * 将输入文件上传至dfs的/input目录下，新建/output/out_dept/deptNumAndSalary为输出目录
 *
 * 运行方法：打成jar包在服务器上执行如下命令
 *
 * hadoop jar jar/mr.jar com.kevin.mapreduce.mr.demo.Q2DeptNumAndAvgSalary /input/dept.txt /input/emp.txt /output/out_dept/deptNumAndSalary
 *
 * creat_user: kevin
 * creat_time: 2018/9/3 23:05
 * email     : kevin_love_it@163.com
 **/
public class Q2DeptNumAndAvgSalary extends Configured implements Tool {
    ...
    public static class ReduceClass extends Reducer<Text, Text, Text, Text> {
        ...
        protected void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
            long sumSalary = 0L;
            long deptNum = 0L;
            //对同一部门的员工工资汇总，同时累加该部门员工数量
            for (Text t : values) {
                sumSalary += Long.parseLong(t.toString());
                deptNum++;
            }
            //输出key为部门名称和value为该部门员工平均工资和人数
            context.write(key, new Text("deptNum:"+deptNum+",avgSalary:"+sumSalary/deptNum));
        }
    }
    ...
    public int run(String[] args) throws Exception {
        ...
        job.setJobName("Q2DeptNumAndAvgSalary");
        job.setJarByClass(Q2DeptNumAndAvgSalary.class);
//        job.setJar(PathConst.JAR_OUTPUT_PATH);
    ...
    public static void main(String[] args) throws Exception{
        int result = ToolRunner.run(new Configuration(), new Q2DeptNumAndAvgSalary(), args);
        System.exit(result);
    }
}
